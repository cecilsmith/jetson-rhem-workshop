...
# recorded in the image history. Override at build time with
# --build-arg, or change the DEVICE_USERNAME / DEVICE_USER_PASSWD repository
# variables in CI. Change them before deploying a device anywhere that matters.
ARG USERNAME=redhat
...
#   printf %s "$RH_USERNAME" > .secrets/rh_username
...
#     --squash -t "${IMAGE_NAME}" -f <this file> .
#
...
RUN useradd -m -d /var/home/$USERNAME -G wheel $USERNAME && \
